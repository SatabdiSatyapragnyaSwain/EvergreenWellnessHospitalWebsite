...
@Component({
  ...
  styleUrl: './sign-up-doctor.component.css'
})
export class SignUpDoctorComponent {
  ...
  constructor(private fb: FormBuilder, private Service: MainserviceService, private router: Router) {
    ...
    this.signupForm = this.fb.group({
      ...
      HospID: ['', [Validators.required]],
      DeptID: ['', [Validators.required]],
      DoctQualification: ['', [Validators.required]],
      DoctSpecialization: ['', [Validators.required]]
  ...
  onSubmit(): void {
    if (this.signupForm.valid) {
      ...
      this.Service.AddDoctorDetails(userData).subscribe(
        (response) => {
          if (response.status) {
            Swal.fire({
              title: 'Success!',
